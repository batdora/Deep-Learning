...
# Validation split
train_images, val_images, train_labels_enc, val_labels_enc = train_test_split(
...
# Load GloVe vectors
labels_glove = np.array([
    np.loadtxt("rabbit_glove.txt"),
    np.loadtxt("yoga_glove.txt"),
    np.loadtxt("hand_glove.txt"),
    np.loadtxt("snowman_glove.txt"),
    np.loadtxt("motorbike_glove.txt")
])

# Define image model (MLP-I)
...
for epoch in range(epochs):
    with tf.GradientTape() as tape:
        anchor_output = image_model(train_images, training=True)
        pos_indices = np.argmax(train_labels_enc, axis=1)
        pos_embeddings = glove_model(labels_glove[pos_indices])

        neg_indices = np.array([
            np.random.choice([j for j in range(num_classes) if j != i])
            for i in pos_indices
        ])
        neg_embeddings = glove_model(labels_glove[neg_indices])
    ...
    # Validation
    val_anchor = image_model(val_images, training=False)
    val_pos_idx = np.argmax(val_labels_enc, axis=1)
...
plt.xlabel("Epochs")
plt.ylabel("Loss")
plt.title("Triplet Loss (TensorFlow Implementation)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
